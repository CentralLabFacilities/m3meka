...
void isr_update_inputs(void)
{
	...
	#endif
        pack_status_out();
	memcpy(pdo_stat,(unsigned char *)&ec_stat_out,sizeof(ec_stat_out));
...
void isr_update_input_pdo(void)
{
	...
	*/
	...
	ec_stat.status[0].adc_ext_a=get_avg_adc(ADC_EXT);
	ec_stat.status[0].adc_ext_b=get_avg_vertx(VERTX_CH_ENC);
        //ec_stat.status[0].debug = vertx_error(VERTX_CH_ENC);
        //ec_stat.status[0].adc_torque=dac_cmd(0); //Torque is proportional to current command
	//ec_stat.status[0].adc_motor_temp=get_avg_adc(ADC_MOTOR_TEMP);
	//ec_stat.status[0].adc_ext_a=get_avg_adc(ADC_EXT);
	//ec_stat.status[0].adc_ext_b=0;
        ...
        //ec_stat.status[0].debug = get_period_HW();;
        ec_stat.status[0].debug = 0;

        //step_qei();

        ec_stat.status[0].qei_on=qei_position_LW();

        //ec_stat.status[0].qei_on=qei_freeze_LW();
	//ec_stat.status[0].qei_period=qei_error();
        //ec_stat.status[0].qei_period = get_period_LW();
        //ec_stat.status[0].qei_period = get_period_16();
        ec_stat.status[0].qei_period = qei_period_t1;


	ec_stat.status[0].qei_rollover=qei_position_HW();
        //ec_stat.status[0].qei_rollover=qei_freeze_HW();
	...
	//ec_stat.status[0].debug = ec_debug[0]; vertx_error        
