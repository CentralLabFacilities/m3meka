...
void setup_interrupt_priorities(void)
{
	...
	_INT0IP = 6;	//Ethercat Interrupt
	_INT2IP = 2;	//SYNC0 Interrupt
	_AD1IP = 5;		//ADC conversion done
	_T1IF = 0;
	_T1IP = 7;		//Timer1 ToDo Remove?
	_DMA2IP = 5;
        _DMA1IP = 6;
        _DMA0IP = 6;
        IPC1bits.IC2IP = 7; // Setup IC2 interrupt priority level index pulse
        _QEI1IP	= 7;	// Quadrature rollover.
	#ifdef USE_BLDC
...
void setup_ports(void)
{
	...
	*/
	...
	//TRISCbits.TRISC1=0;		//RC1	OUTPUT	PIN26	SPI_CLK_ENC
        TRISBbits.TRISB10=0;		//RB3	OUTPUT	PIN24	SPI_SS_ENC/
	TRISBbits.TRISB11=0;		//RC1	OUTPUT	PIN26	SPI_CLK_ENC
...
void setup_peripheral_pin_select(void)
{
	...
	//RPINR0bits.INT1R=19;	//(SYNC1) INT1 ON RP19 :					RPINR0
        RPINR0bits.INT1R=5;	//(SYNC1) INT1 ON RP5 :					RPINR0  BREAKBEAM
	...
	RPINR15bits.INDX1R=8;		//ENCODER_I ON RP8 :
        ...
        // INTCON2 = 0x0020;       /*Setup INT1pins to interupt */
                                /*on falling edge */
