...
ServiceServer M3Humanoid::RosInitCmd(NodeHandle * node_handle)
{	
	//return node_handle->advertiseService(GetName()+"_cmd",&M3Humanoid::RosCallbackCmd, this);
	return node_handle->advertiseService("humanoid_cmd",&M3Humanoid::RosCallbackCmd, this);
}


ServiceServer M3Humanoid::RosInitStatus(NodeHandle * node_handle)
{
	//return node_handle->advertiseService(GetName()+"_status",&M3Humanoid::RosCallbackStatus, this);
	return node_handle->advertiseService("humanoid_status",&M3Humanoid::RosCallbackStatus, this);
}

ServiceServer M3Humanoid::RosInitParam(NodeHandle * node_handle)
{
	//return node_handle->advertiseService(GetName()+"_param",&M3Humanoid::RosCallbackParam, this);
	return node_handle->advertiseService("humanoid_param",&M3Humanoid::RosCallbackParam, this);
}

bool M3Humanoid::RosCallbackCmd(m3meka_msgs::M3HumanoidCmd::Request  &req, m3meka_msgs::M3HumanoidCmd::Response &res)
{
  command.set_enable_motor(req.enable_motor);
  if ((M3Chain)req.chain == TORSO)
   {
	if(torso)
	{
	  for (int i=0; i<torso->GetNumDof(); i++)
	  {	    
	    command.mutable_torso()->set_tq_desired(i,req.tq_desired[i]);
	    command.mutable_torso()->set_ctrl_mode(i,(JOINT_ARRAY_MODE)req.ctrl_mode[i]);
	    command.mutable_torso()->set_q_stiffness(i,req.q_stiffness[i]);
	    command.mutable_torso()->set_q_desired(i,req.q_desired[i]);
	    command.mutable_torso()->set_qdot_desired(i,req.qdot_desired[i]);
	    command.mutable_torso()->set_q_slew_rate(i,req.q_slew_rate[i]);
	    command.mutable_torso()->set_pwm_desired(i,req.pwm_desired[i]);
	  }  
	}
   }
   else if ((M3Chain)req.chain == RIGHT_ARM)
   {
	if(right_arm)
	{
	 for (int i=0; i<right_arm->GetNumDof(); i++)
	  {	
	    command.mutable_right_arm()->set_tq_desired(i,req.tq_desired[i]);
	    command.mutable_right_arm()->set_ctrl_mode(i,(JOINT_ARRAY_MODE)req.ctrl_mode[i]);	  
	    command.mutable_right_arm()->set_q_stiffness(i,req.q_stiffness[i]);
	    command.mutable_right_arm()->set_q_desired(i,req.q_desired[i]);	    
	    command.mutable_right_arm()->set_qdot_desired(i,req.qdot_desired[i]);
	    command.mutable_right_arm()->set_q_slew_rate(i,req.q_slew_rate[i]);
	    command.mutable_right_arm()->set_pwm_desired(i,req.pwm_desired[i]);
	  }  
	}
   }
   else if ((M3Chain)req.chain == LEFT_ARM)
   {
	if(left_arm)
	{
	 for (int i=0; i<left_arm->GetNumDof(); i++)
	  {
	    command.mutable_left_arm()->set_tq_desired(i,req.tq_desired[i]);
	    command.mutable_left_arm()->set_ctrl_mode(i,(JOINT_ARRAY_MODE)req.ctrl_mode[i]);
	    command.mutable_left_arm()->set_q_stiffness(i,req.q_stiffness[i]);
	    command.mutable_left_arm()->set_q_desired(i,req.q_desired[i]);
	    command.mutable_left_arm()->set_qdot_desired(i,req.qdot_desired[i]);
	    command.mutable_left_arm()->set_q_slew_rate(i,req.q_slew_rate[i]);
	    command.mutable_left_arm()->set_pwm_desired(i,req.pwm_desired[i]);
	  }  
	}	
   }
   else if ((M3Chain)req.chain == HEAD)
   {
	if(head)
	{
	  for (int i=0; i<head->GetNumDof(); i++)
	  {
	    command.mutable_head()->set_tq_desired(i,req.tq_desired[i]);
	    command.mutable_head()->set_ctrl_mode(i,(JOINT_ARRAY_MODE)req.ctrl_mode[i]);
	    command.mutable_head()->set_q_stiffness(i,req.q_stiffness[i]);
	    command.mutable_head()->set_q_desired(i,req.q_desired[i]);
	    command.mutable_head()->set_qdot_desired(i,req.qdot_desired[i]);
	    command.mutable_head()->set_q_slew_rate(i,req.q_slew_rate[i]);
	    command.mutable_head()->set_pwm_desired(i,req.pwm_desired[i]);
	  }  
	}	
   }   	
...
bool M3Humanoid::RosCallbackParam(m3meka_msgs::M3HumanoidParam::Request  &req, m3meka_msgs::M3HumanoidParam::Response &res)
{
   if ((M3Chain)req.chain == TORSO)
   {
	if(torso)
	{
	  param.mutable_torso()->set_payload_mass(req.payload_mass);
	  for (int i=0; i<3; i++)
	      param.mutable_torso()->set_payload_com(i,req.payload_com[i]);
	  for (int i=0; i<6; i++)
	      param.mutable_torso()->set_payload_inertia(i,req.payload_inertia[i]);
	  param.mutable_torso()->set_use_velocities(req.use_velocities);
	  param.mutable_torso()->set_use_accelerations(req.use_accelerations);	  
	}
   }
   else if ((M3Chain)req.chain == RIGHT_ARM)
   {
	if(right_arm)
	{
	  param.mutable_right_arm()->set_payload_mass(req.payload_mass);
	  for (int i=0; i<3; i++)
	      param.mutable_right_arm()->set_payload_com(i,req.payload_com[i]);
	  for (int i=0; i<6; i++)
	      param.mutable_right_arm()->set_payload_inertia(i,req.payload_inertia[i]);
	  param.mutable_right_arm()->set_use_velocities(req.use_velocities);
	  param.mutable_right_arm()->set_use_accelerations(req.use_accelerations);	  
	}
   }
   else if ((M3Chain)req.chain == LEFT_ARM)
   {
	if(left_arm)
	{
	  param.mutable_left_arm()->set_payload_mass(req.payload_mass);
	  for (int i=0; i<3; i++)
	      param.mutable_left_arm()->set_payload_com(i,req.payload_com[i]);
	  for (int i=0; i<6; i++)
	      param.mutable_left_arm()->set_payload_inertia(i,req.payload_inertia[i]);
	  param.mutable_left_arm()->set_use_velocities(req.use_velocities);
	  param.mutable_left_arm()->set_use_accelerations(req.use_accelerations);	
	}	
   }
   else if ((M3Chain)req.chain == HEAD)
   {
	if(head)
	{
	  param.mutable_head()->set_payload_mass(req.payload_mass);
	  for (int i=0; i<3; i++)
	      param.mutable_head()->set_payload_com(i,req.payload_com[i]);
	  for (int i=0; i<6; i++)
	      param.mutable_head()->set_payload_inertia(i,req.payload_inertia[i]);
	  param.mutable_head()->set_use_velocities(req.use_velocities);
	  param.mutable_head()->set_use_accelerations(req.use_accelerations);	
	}	
   }   	
...
bool M3Humanoid::RosCallbackStatus(m3meka_msgs::M3HumanoidStatus::Request  &req, m3meka_msgs::M3HumanoidStatus::Response &res)
{
   if ((M3Chain)req.chain == TORSO)
   {
	if(torso)
	{ 
	  res.torque.resize(torso->GetNumDof());
	  res.torquedot.resize(torso->GetNumDof());
	  res.theta.resize(torso->GetNumDof());
	  res.thetadot.resize(torso->GetNumDof());
	  res.thetadotdot.resize(torso->GetNumDof());
	  res.pwm_cmd.resize(torso->GetNumDof());
	  res.G.resize(torso->GetNumDof());
	  res.J.resize(6*torso->GetNumDof());
	  
	  res.completed_spline_idx = status.torso().completed_spline_idx();
	  for (int i=0; i<torso->GetNumDof(); i++)
	  {
	    res.torque[i] = status.torso().torque(i);
	    //res.torque.push_back(status.torso().torque(i));
	    res.torquedot[i] = status.torso().torquedot(i);
	    res.theta[i] = status.torso().theta(i);
	    res.thetadot[i] = status.torso().thetadot(i);
	    res.thetadotdot[i] = status.torso().thetadotdot(i);
	    res.pwm_cmd[i] = status.torso().pwm_cmd(i);
	    res.G[i] = status.torso().g(i);
	  }
	  for (int i = 0; i < 3; i++)
	  {
	      res.end_pos[i] = status.torso().end_pos(i);
	  }
	  for (int i = 0; i < 9; i++)
	  {
	      res.end_rot[i] = status.torso().end_rot(i);
	  }
	  for (int i = 0; i < 6; i++)
	  {
	      res.end_twist[i] = status.torso().end_twist(i);
	  }
	  for (int i = 0; i < (6*torso->GetNumDof()); i++)
	  {
	      res.J[i] = status.torso().j(i);
	  }
	}
   }
   else if ((M3Chain)req.chain == RIGHT_ARM)
   {
	if(right_arm)
	{
	  res.torque.resize(right_arm->GetNumDof());
	  res.torquedot.resize(right_arm->GetNumDof());
	  res.theta.resize(right_arm->GetNumDof());
	  res.thetadot.resize(right_arm->GetNumDof());
	  res.thetadotdot.resize(right_arm->GetNumDof());
	  res.pwm_cmd.resize(right_arm->GetNumDof());
	  res.G.resize(right_arm->GetNumDof());
	  res.J.resize(6*right_arm->GetNumDof());
	  
	  res.completed_spline_idx = status.right_arm().completed_spline_idx();
	  for (int i=0; i<right_arm->GetNumDof(); i++)
	  {
	    res.torque[i] = status.right_arm().torque(i);
	    res.torquedot[i] = status.right_arm().torquedot(i);
	    res.theta[i] = status.right_arm().theta(i);
	    res.thetadot[i] = status.right_arm().thetadot(i);
	    res.thetadotdot[i] = status.right_arm().thetadotdot(i);
	    res.pwm_cmd[i] = status.right_arm().pwm_cmd(i);
	    res.G[i] = status.right_arm().g(i);
	  }
	  for (int i = 0; i < 3; i++)
	  {
	      res.end_pos[i] = status.right_arm().end_pos(i);
	  }
	  for (int i = 0; i < 9; i++)
	  {
	      res.end_rot[i] = status.right_arm().end_rot(i);
	  }
	  for (int i = 0; i < 6; i++)
	  {
	      res.end_twist[i] = status.right_arm().end_twist(i);
	  }
	  for (int i = 0; i < (6*right_arm->GetNumDof()); i++)
	  {
	      res.J[i] = status.right_arm().j(i);
	  }
	}
   }
   else if ((M3Chain)req.chain == LEFT_ARM)
   {
	if(left_arm)
	{
	  res.torque.resize(left_arm->GetNumDof());
	  res.torquedot.resize(left_arm->GetNumDof());
	  res.theta.resize(left_arm->GetNumDof());
	  res.thetadot.resize(left_arm->GetNumDof());
	  res.thetadotdot.resize(left_arm->GetNumDof());
	  res.pwm_cmd.resize(left_arm->GetNumDof());
	  res.G.resize(left_arm->GetNumDof());
	  res.J.resize(6*left_arm->GetNumDof());
	  
	  res.completed_spline_idx = status.left_arm().completed_spline_idx();
	  for (int i=0; i<left_arm->GetNumDof(); i++)
	  {
	    res.torque[i] = status.left_arm().torque(i);
	    res.torquedot[i] = status.left_arm().torquedot(i);
	    res.theta[i] = status.left_arm().theta(i);
	    res.thetadot[i] = status.left_arm().thetadot(i);
	    res.thetadotdot[i] = status.left_arm().thetadotdot(i);
	    res.pwm_cmd[i] = status.left_arm().pwm_cmd(i);
	    res.G[i] = status.left_arm().g(i);
	  }
	  for (int i = 0; i < 3; i++)
	  {
	      res.end_pos[i] = status.left_arm().end_pos(i);
	  }
	  for (int i = 0; i < 9; i++)
	  {
	      res.end_rot[i] = status.left_arm().end_rot(i);
	  }
	  for (int i = 0; i < 6; i++)
	  {
	      res.end_twist[i] = status.left_arm().end_twist(i);
	  }
	  for (int i = 0; i < (6*left_arm->GetNumDof()); i++)
	  {
	      res.J[i] = status.left_arm().j(i);
	  }
	}	
   }
   else if ((M3Chain)req.chain == HEAD)
   {
	if(head)
	{
	  res.torque.resize(head->GetNumDof());
	  res.torquedot.resize(head->GetNumDof());
	  res.theta.resize(head->GetNumDof());
	  res.thetadot.resize(head->GetNumDof());
	  res.thetadotdot.resize(head->GetNumDof());
	  res.pwm_cmd.resize(head->GetNumDof());
	  res.G.resize(head->GetNumDof());
	  res.J.resize(6*head->GetNumDof());
	  
	  res.completed_spline_idx = status.head().completed_spline_idx();
	  for (int i=0; i<head->GetNumDof(); i++)
	  {
	    res.torque[i] = status.head().torque(i);
	    res.torquedot[i] = status.head().torquedot(i);
	    res.theta[i] = status.head().theta(i);
	    res.thetadot[i] = status.head().thetadot(i);
	    res.thetadotdot[i] = status.head().thetadotdot(i);
	    res.pwm_cmd[i] = status.head().pwm_cmd(i);
	    res.G[i] = status.head().g(i);
	  }
	  for (int i = 0; i < 3; i++)
	  {
	      res.end_pos[i] = status.head().end_pos(i);
	  }
	  for (int i = 0; i < 9; i++)
	  {
	      res.end_rot[i] = status.head().end_rot(i);
	  }
	  for (int i = 0; i < 6; i++)
	  {
	      res.end_twist[i] = status.head().end_twist(i);
	  }
	  for (int i = 0; i < (6*head->GetNumDof()); i++)
	  {
	      res.J[i] = status.head().j(i);
	  }
	}	
   }   	
	return true;
